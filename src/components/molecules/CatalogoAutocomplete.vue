  ...
  <div class="relative" ref="wrapperRef">
    ...
    >
      ...
      >
        ...
        <CircleCheck v-if="modelValue === item.id" class="size-4 text-success flex-shrink-0" />
      </button>
...
const selectedLabel = computed(() => {
  ...
  return allItems.value.find((it) => it.id === props.modelValue)?.nombre || ''
})

const inputClasses = [
  'w-full rounded-lg border border-base-300 bg-base-100 px-3 py-2 text-sm transition-colors',
...
function onBlur() {
  setTimeout(() => {
    open.value = false
    if (props.modelValue != null && !searchText.value) searchText.value = selectedLabel.value
  }, 150)
}
function moveDown() {
...
function showSuggestDialog() {
  open.value = false
  emit('sugerir', { tabla: props.tablaDestino || props.catalogo, nombre: searchText.value })
}
...
async function loadCatalog() {
  ...
  try {
    const res = props.endpoint
      ? await api.get(props.endpoint)
      : await catalogService.getCatalogo(props.catalogo)
